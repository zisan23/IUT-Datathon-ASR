...
@registry.register_lr_scheduler("linear_warmup_cosine_long_long_tail_lr")
class LinearWarmupCosineLongLongTailLRScheduler:
    def __init__(
            self,
            optimizer,
            max_epoch,
    ...
    def step(self, cur_epoch, cur_step):
        ...
        elif total_steps <= self.max_iters // 3:
            cosine_lr_schedule(
                epoch=total_steps,
                optimizer=self.optimizer,
                max_epoch=self.max_iters // 3,
                init_lr=self.init_lr,
                min_lr=self.min_lr,
            )
        elif total_steps <= self.max_iters // 3 * 2:
            cosine_lr_schedule(
                epoch=self.max_iters // 3,
                optimizer=self.optimizer,
                max_epoch=self.max_iters // 3,
                init_lr=self.init_lr,
                min_lr=self.min_lr,
            )
        else:  # total_steps > self.max_iters // 3 * 2
            cosine_lr_schedule(
                epoch=total_steps - self.max_iters // 3 * 2,
                optimizer=self.optimizer,
                max_epoch=self.max_iters // 3,
                init_lr=self.min_lr,
                min_lr=0,
...
@registry.register_lr_scheduler("linear_warmup_cosine_3_long_tail_lr")
